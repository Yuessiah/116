...
int upperBound, lowerBound, task_itv;
RGBTRIPLE *upperRecv;
RGBTRIPLE *lowerRecv;
...
int main(int argc,char *argv[])
{
	...
	task_itv = (my_rank != comm_sz-1)? bmpInfo.biHeight/comm_sz : bmpInfo.biHeight - (bmpInfo.biHeight / comm_sz)*(comm_sz-1);
	upperBound = my_rank * (bmpInfo.biHeight/comm_sz);
	lowerBound = upperBound+task_itv - 1;

	for(int count = 0; count < NSmooth; count++){
		swap(BMPSaveData,BMPData);

		MPI_Send(BMPData[upperBound], bmpInfo.biWidth*3, MPI_BYTE, (my_rank-1+comm_sz)%comm_sz, 1, MPI_COMM_WORLD);
		MPI_Send(BMPData[lowerBound], bmpInfo.biWidth*3, MPI_BYTE, (my_rank+1)%comm_sz        , 2, MPI_COMM_WORLD);
		MPI_Recv(upperRecv, bmpInfo.biWidth*3, MPI_BYTE, (my_rank-1+comm_sz)%comm_sz, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		MPI_Recv(lowerRecv, bmpInfo.biWidth*3, MPI_BYTE, (my_rank+1)%comm_sz        , 1, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		for(int i = upperBound; i <= lowerBound; i++)
			for(int j = 0; j < bmpInfo.biWidth; j++){
				...
				if (i == upperBound) Top  = upperRecv[j];
	...
	MPI_Gather(BMPSaveData[upperBound], task_itv*bmpInfo.biWidth*3, MPI_BYTE, BMPSaveData[0], task_itv*bmpInfo.biWidth*3, MPI_BYTE, 0, MPI_COMM_WORLD);

	if (my_rank == 0 && !saveBMP(outfileName))
